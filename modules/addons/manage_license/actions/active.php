...
if ($vars["ip"]==""){
    $result=DB::table("tblhosting")->where("id",$vars["serviceid"])->first();
    DB::table("mod_manage_license_ap")->where("id",$vars["id"])->update([
        'ip'=>$result->dedicatedip
    ]);
    $vars["ip"]=$result->dedicatedip;
}
...
$actions->params["params"]["userid"] = $vars["userid"];
$actions->params["params"]["username"] = $vars["licensekay"];
$actions->params["params"]["accountid"] = $vars["serviceid"];
...
if ($res->result== "error"){
    ...
    DB::table("mod_manage_license_ap")->where("id",$vars["id"])->update([
        "status"=>"Active",
        "licensekay"=> $res->response->licensekey,
    ]);
    echo $response;
}